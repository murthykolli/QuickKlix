<%-- 
    Document   : tm_MaritalStatus_Creation
    Created on : 7 Aug, 2014, 12:20:56 PM
    Author     : murthyk
--%>
...
<html>
    ...
    function checkAll(){
    if(document.maritalStatusFormName.selectAll.checked== true){
    for(var i=0; i < document.maritalStatusFormName.selectedMaritalStatusCategories.length; i++){
    document.maritalStatusFormName.selectedMaritalStatusCategories[i].checked=true;
    } }
    else{
    for(var i=0; i < document.maritalStatusFormName.selectedMaritalStatusCategories.length; i++){
    document.maritalStatusFormName.selectedMaritalStatusCategories[i].checked=false;
    } } }

    function checkStateAll(passStateName){
    var stateChecked = document.getElementById(passStateName).checked;
    var len=document.maritalStatusFormName.selectedMaritalStatusCategories.length;

    if(stateChecked== true){
    if(len==undefined){
    document.maritalStatusFormName.selectedMaritalStatusCategories.checked=true;
    }
    for(var i=0; i < document.maritalStatusFormName.selectedMaritalStatusCategories.length; i++){
    if (document.maritalStatusFormName.selectedMaritalStatusCategories[i].className.indexOf(passStateName) != -1){
    document.maritalStatusFormName.selectedMaritalStatusCategories[i].checked = true;
    } }}
     
    else{
    if(len==undefined && document.maritalStatusFormName.selectedMaritalStatusCategories.checked==true ){
    document.maritalStatusFormName.selectedMaritalStatusCategories.checked=false;
    }
    ...
    var selectedMaritalStatusCategoriess = "";
    for (var k = 0; k < document.maritalStatusFormName.selectedMaritalStatusCategories.length; k++){
    if (document.maritalStatusFormName.selectedMaritalStatusCategories[k].checked) {
    selectedMaritalStatusCategoriess = document.maritalStatusFormName.selectedMaritalStatusCategories[k].value;
    } }
    if (document.maritalStatusFormName.selectedMaritalStatusCategories.checked) {
    selectedMaritalStatusCategoriess = document.maritalStatusFormName.selectedMaritalStatusCategories.value;
    }
    if (selectedMaritalStatusCategoriess == "") {
    ...
    <body>
        ...
        <div style="margin-top:20px;margin-left: 30px;"><font style="font-family: Calibri;font-size: 22px;font-weight: bold;color:#6f6d6d;">Select by <font color="#00a9f1">Marital Status</font> Dimension - <font color="#00a9f1">Categories</font></font></div><br/>
        <form name="maritalStatusFormName" action="maritalStatusSaving" method="post">
        ...
        <%
        if(session.getAttribute("MaritalStatus_Map") != null){
        Map sortedMap = (Map)session.getAttribute("MaritalStatus_Map");
...
//        Collections.sort(dimensionNameAndCountList);
        ...
        <td><input type="checkbox" name="selectedMaritalStatusCategories" class="<%=dimensionName%>" value="<%= pumaCount%>"/></td>
