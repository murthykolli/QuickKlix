<%-- 
    Document   : tm_MigrationStatus_Creation
    Created on : 7 Aug, 2014, 12:22:07 PM
    Author     : murthyk
--%>
...
<html>
    ...
    function checkAll(){
    if(document.migrationStatusFormName.selectAll.checked== true){
    for(var i=0; i < document.migrationStatusFormName.selectedMigrationStatusCategories.length; i++){
    document.migrationStatusFormName.selectedMigrationStatusCategories[i].checked=true;
    } }
    else{
    for(var i=0; i < document.migrationStatusFormName.selectedMigrationStatusCategories.length; i++){
    document.migrationStatusFormName.selectedMigrationStatusCategories[i].checked=false;
    } } }

    function checkStateAll(passStateName){
    var stateChecked = document.getElementById(passStateName).checked;
    var len=document.migrationStatusFormName.selectedMigrationStatusCategories.length;

    if(stateChecked== true){
    if(len==undefined){
    document.migrationStatusFormName.selectedMigrationStatusCategories.checked=true;
    }
    for(var i=0; i < document.migrationStatusFormName.selectedMigrationStatusCategories.length; i++){
    if (document.migrationStatusFormName.selectedMigrationStatusCategories[i].className.indexOf(passStateName) != -1){
    document.migrationStatusFormName.selectedMigrationStatusCategories[i].checked = true;
    } }}
     
    else{
    if(len==undefined && document.migrationStatusFormName.selectedMigrationStatusCategories.checked==true ){
    document.migrationStatusFormName.selectedMigrationStatusCategories.checked=false;
    }
    ...
    var selectedMigrationStatusCategoriess = "";
    for (var k = 0; k < document.migrationStatusFormName.selectedMigrationStatusCategories.length; k++){
    if (document.migrationStatusFormName.selectedMigrationStatusCategories[k].checked) {
    selectedMigrationStatusCategoriess = document.migrationStatusFormName.selectedMigrationStatusCategories[k].value;
    } }
    if (document.migrationStatusFormName.selectedMigrationStatusCategories.checked) {
    selectedMigrationStatusCategoriess = document.migrationStatusFormName.selectedMigrationStatusCategories.value;
    }
    if (selectedMigrationStatusCategoriess == "") {
    ...
    <body>
        ...
        <div style="margin-top:20px;margin-left: 30px;"><font style="font-family: Calibri;font-size: 22px;font-weight: bold;color:#6f6d6d;">Select by <font color="#00a9f1">MigrationStatus</font> Dimension - <font color="#00a9f1">Categories</font></font></div><br/>
        <form name="migrationStatusFormName" action="migrationSaving" method="post">
        ...
        <%
        if(session.getAttribute("MigrationStatus_Map") != null){
        Map sortedMap = (Map)session.getAttribute("MigrationStatus_Map");
        ...
        <div class="mainBox" style="margin-left: 40px;margin-top:34px;background: white;min-height: 55px;width: 880px;border-radius: 12px 10px 0px 0px;height: auto;border: solid 2px #595959;">
        ...
        <div style="width:860px; height: 55px;margin-left: 7px;margin-top: 13px; overflow:auto;">
...
//        Collections.sort(dimensionNameAndCountList);
        ...
        <td><input type="checkbox" name="selectedMigrationStatusCategories" class="<%=dimensionName%>" value="<%= pumaCount%>"/></td>
        ...
        <tr><td><button type="submit" name="method:migrationDimensionSaving" onMouseOver="this.style.color='#00a9f1';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerWMOImg.png';" onMouseOut="this.style.color='white';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerImg.png';" style="background: #595959;border-radius: 7px;cursor: pointer; border: 1px solid #595959; color: white; height: 36px; font: bold 1.4em/1em Verdana,Geneva,Arial,Helvetica,sans-serif;box-shadow: 3px 3px 1px #999999;" onclick="return citySelection();"><font style="top: -2px;position: relative;">NEXT</font> <img id="NextButtonImg1" src="/QuickKlix/images/SSImgs/TextPointerImg.png" style="position: relative;top: 1px;"/></button></td></tr>
        </table>
        </div>
