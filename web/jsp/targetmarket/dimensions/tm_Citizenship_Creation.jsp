<%-- 
    Document   : tm_Citizenship_Creation
    Created on : 7 Aug, 2014, 12:17:03 PM
    Author     : murthyk
--%>
...
<html>
    ...
    function checkAll(){
    if(document.citizenshipFormName.selectAll.checked== true){
    for(var i=0; i < document.citizenshipFormName.selectedCitizenshipCategories.length; i++){
    document.citizenshipFormName.selectedCitizenshipCategories[i].checked=true;
    } }
    else{
    for(var i=0; i < document.citizenshipFormName.selectedCitizenshipCategories.length; i++){
    document.citizenshipFormName.selectedCitizenshipCategories[i].checked=false;
    } } }

    function checkStateAll(passStateName){
    var stateChecked = document.getElementById(passStateName).checked;
    var len=document.citizenshipFormName.selectedCitizenshipCategories.length;

    if(stateChecked== true){
    if(len==undefined){
    document.citizenshipFormName.selectedCitizenshipCategories.checked=true;
    }
    for(var i=0; i < document.citizenshipFormName.selectedCitizenshipCategories.length; i++){
    if (document.citizenshipFormName.selectedCitizenshipCategories[i].className.indexOf(passStateName) != -1){
    document.citizenshipFormName.selectedCitizenshipCategories[i].checked = true;
    } }}
     
    else{
    if(len==undefined && document.citizenshipFormName.selectedCitizenshipCategories.checked==true ){
    document.citizenshipFormName.selectedCitizenshipCategories.checked=false;
    }
    ...
    var selectedCitizenshipCategoriess = "";
    for (var k = 0; k < document.citizenshipFormName.selectedCitizenshipCategories.length; k++){
    if (document.citizenshipFormName.selectedCitizenshipCategories[k].checked) {
    selectedCitizenshipCategoriess = document.citizenshipFormName.selectedCitizenshipCategories[k].value;
    } }
    if (document.citizenshipFormName.selectedCitizenshipCategories.checked) {
    selectedCitizenshipCategoriess = document.citizenshipFormName.selectedCitizenshipCategories.value;
    }
    if (selectedCitizenshipCategoriess == "") {
    ...
    <body>
        ...
        <div style="margin-top:20px;margin-left: 30px;"><font style="font-family: Calibri;font-size: 22px;font-weight: bold;color:#6f6d6d;">Select by <font color="#00a9f1">Citizenship</font> Dimension - <font color="#00a9f1">Categories</font></font></div><br/>
        <form name="citizenshipFormName" action="citizenshipSaving" method="post">
        ...
        <%
        if(session.getAttribute("Citizenship_Map") != null){
        Map sortedMap = (Map)session.getAttribute("Citizenship_Map");
        ...
        <div class="mainBox" style="margin-left: 40px;margin-top:34px;background: white;min-height: 100px;width: 880px;border-radius: 12px 10px 0px 0px;height: auto;border: solid 2px #595959;">
...
//        Collections.sort(dimensionNameAndCountList);
        ...
        <td><input type="checkbox" name="selectedCitizenshipCategories" class="<%=dimensionName%>" value="<%= pumaCount%>"/></td>
        ...
        <tr><td><button type="submit" name="method:citizenshipDimensionSaving" onMouseOver="this.style.color='#00a9f1';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerWMOImg.png';" onMouseOut="this.style.color='white';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerImg.png';" style="background: #595959;border-radius: 7px;cursor: pointer; border: 1px solid #595959; color: white; height: 36px; font: bold 1.4em/1em Verdana,Geneva,Arial,Helvetica,sans-serif;box-shadow: 3px 3px 1px #999999;" onclick="return citySelection();"><font style="top: -2px;position: relative;">NEXT</font> <img id="NextButtonImg1" src="/QuickKlix/images/SSImgs/TextPointerImg.png" style="position: relative;top: 1px;"/></button></td></tr>
        </table>
        </div>
