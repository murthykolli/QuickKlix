<%-- 
    Document   : tm_ClassOfWorker_Creation
    Created on : 7 Aug, 2014, 12:17:43 PM
    Author     : murthyk
--%>
...
<html>
    ...
    function checkAll(){
    if(document.classOfWorkerFormName.selectAll.checked== true){
    for(var i=0; i < document.classOfWorkerFormName.selectedClassOfWorkerCategories.length; i++){
    document.classOfWorkerFormName.selectedClassOfWorkerCategories[i].checked=true;
    } }
    else{
    for(var i=0; i < document.classOfWorkerFormName.selectedClassOfWorkerCategories.length; i++){
    document.classOfWorkerFormName.selectedClassOfWorkerCategories[i].checked=false;
    } } }

    function checkStateAll(passStateName){
    var stateChecked = document.getElementById(passStateName).checked;
    var len=document.classOfWorkerFormName.selectedClassOfWorkerCategories.length;

    if(stateChecked== true){
    if(len==undefined){
    document.classOfWorkerFormName.selectedClassOfWorkerCategories.checked=true;
    }
    for(var i=0; i < document.classOfWorkerFormName.selectedClassOfWorkerCategories.length; i++){
    if (document.classOfWorkerFormName.selectedClassOfWorkerCategories[i].className.indexOf(passStateName) != -1){
    document.classOfWorkerFormName.selectedClassOfWorkerCategories[i].checked = true;
    } }}
     
    else{
    if(len==undefined && document.classOfWorkerFormName.selectedClassOfWorkerCategories.checked==true ){
    document.classOfWorkerFormName.selectedClassOfWorkerCategories.checked=false;
    }
    ...
    var selectedClassOfWorkerCategoriess = "";
    for (var k = 0; k < document.classOfWorkerFormName.selectedClassOfWorkerCategories.length; k++){
    if (document.classOfWorkerFormName.selectedClassOfWorkerCategories[k].checked) {
    selectedClassOfWorkerCategoriess = document.classOfWorkerFormName.selectedClassOfWorkerCategories[k].value;
    } }
    if (document.classOfWorkerFormName.selectedClassOfWorkerCategories.checked) {
    selectedClassOfWorkerCategoriess = document.classOfWorkerFormName.selectedClassOfWorkerCategories.value;
    }
    if (selectedClassOfWorkerCategoriess == "") {
    ...
    <body>
        ...
        <div style="margin-top:20px;margin-left: 30px;"><font style="font-family: Calibri;font-size: 22px;font-weight: bold;color:#6f6d6d;">Select by <font color="#00a9f1">Class Of Worker</font> Dimension - <font color="#00a9f1">Categories</font></font></div><br/>
        <form name="classOfWorkerFormName" action="classOfWorkerSaving" method="post">
        ...
        <%
        if(session.getAttribute("ClassOfWorker_Map") != null){
        Map sortedMap = (Map)session.getAttribute("ClassOfWorker_Map");
        ...
        <div class="mainBox" style="margin-left: 40px;margin-top:34px;background: white;min-height: 250px;width: 880px;border-radius: 12px 10px 0px 0px;height: auto;border: solid 2px #595959;">
        ...
        <div style="width:860px; height: 250px;margin-left: 7px;margin-top: 13px; overflow:auto;">
...
//        Collections.sort(dimensionNameAndCountList);
        ...
        <td><input type="checkbox" name="selectedClassOfWorkerCategories" class="<%=dimensionName%>" value="<%= pumaCount%>"/></td>
        ...
        <tr><td><button type="submit" name="method:classOfWorkerDimensionSaving" onMouseOver="this.style.color='#00a9f1';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerWMOImg.png';" onMouseOut="this.style.color='white';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerImg.png';" style="background: #595959;border-radius: 7px;cursor: pointer; border: 1px solid #595959; color: white; height: 36px; font: bold 1.4em/1em Verdana,Geneva,Arial,Helvetica,sans-serif;box-shadow: 3px 3px 1px #999999;" onclick="return citySelection();"><font style="top: -2px;position: relative;">NEXT</font> <img id="NextButtonImg1" src="/QuickKlix/images/SSImgs/TextPointerImg.png" style="position: relative;top: 1px;"/></button></td></tr>
        </table>
        </div>
