<%-- 
    Document   : tm_Race_Creation
    Created on : 7 Aug, 2014, 12:22:51 PM
    Author     : murthyk
--%>
...
<html>
    ...
    function checkAll(){
    if(document.raceFormName.selectAll.checked== true){
    for(var i=0; i < document.raceFormName.selectedRaceCategories.length; i++){
    document.raceFormName.selectedRaceCategories[i].checked=true;
    } }
    else{
    for(var i=0; i < document.raceFormName.selectedRaceCategories.length; i++){
    document.raceFormName.selectedRaceCategories[i].checked=false;
    } } }

    function checkStateAll(passStateName){
    var stateChecked = document.getElementById(passStateName).checked;
    var len=document.raceFormName.selectedRaceCategories.length;

    if(stateChecked== true){
    if(len==undefined){
    document.raceFormName.selectedRaceCategories.checked=true;
    }
    for(var i=0; i < document.raceFormName.selectedRaceCategories.length; i++){
    if (document.raceFormName.selectedRaceCategories[i].className.indexOf(passStateName) != -1){
    document.raceFormName.selectedRaceCategories[i].checked = true;
    } }}
     
    else{
    if(len==undefined && document.raceFormName.selectedRaceCategories.checked==true ){
    document.raceFormName.selectedRaceCategories.checked=false;
    }
    ...
    var selectedRaceCategoriess = "";
    for (var k = 0; k < document.raceFormName.selectedRaceCategories.length; k++){
    if (document.raceFormName.selectedRaceCategories[k].checked) {
    selectedRaceCategoriess = document.raceFormName.selectedRaceCategories[k].value;
    } }
    if (document.raceFormName.selectedRaceCategories.checked) {
    selectedRaceCategoriess = document.raceFormName.selectedRaceCategories.value;
    }
    if (selectedRaceCategoriess == "") {
    ...
    <body>
        ...
        <div style="margin-top:20px;margin-left: 30px;"><font style="font-family: Calibri;font-size: 22px;font-weight: bold;color:#6f6d6d;">Select by <font color="#00a9f1">Race</font> Dimension - <font color="#00a9f1">Categories</font></font></div><br/>
        <form name="raceFormName" action="raceSaving" method="post">
        ...
        <%
        if(session.getAttribute("Race_Map") != null){
        Map sortedMap = (Map)session.getAttribute("Race_Map");
...
//        Collections.sort(dimensionNameAndCountList);
        ...
        <td><input type="checkbox" name="selectedRaceCategories" class="<%=dimensionName%>" value="<%= pumaCount%>"/></td>
        ...
        <tr><td><button type="submit" name="method:raceDimensionSaving" onMouseOver="this.style.color='#00a9f1';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerWMOImg.png';" onMouseOut="this.style.color='white';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerImg.png';" style="background: #595959;border-radius: 7px;cursor: pointer; border: 1px solid #595959; color: white; height: 36px; font: bold 1.4em/1em Verdana,Geneva,Arial,Helvetica,sans-serif;box-shadow: 3px 3px 1px #999999;" onclick="return citySelection();"><font style="top: -2px;position: relative;">NEXT</font> <img id="NextButtonImg1" src="/QuickKlix/images/SSImgs/TextPointerImg.png" style="position: relative;top: 1px;"/></button></td></tr>
        </table>
        </div>
