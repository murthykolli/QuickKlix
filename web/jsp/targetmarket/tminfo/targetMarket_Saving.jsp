<%-- 
    Document   : targetmarketsaving
    Created on : 3 Jun, 2014, 4:51:50 PM
    Author     : murthyk
--%>
...
function Pager(tableName, itemsPerPage) {
 ...
 function onLoadProperties()
    ...
    function changeColor(chk){
                if(chk.checked==true || document.tmFormName.combinationalName[chk].checked==true){
                    var i;
            ...
            function highlightTableRows(tableId) {
                ...
                for (var i=0; i < rows.length; i++) {
                    rows[i].onmouseover = function() {
                        ...
                        var cell=row.getElementsByTagName('td');
                        if((rows.length==2 && document.tmFormName.combinationalName.checked==false) || (document.tmFormName.combinationalName[j-1].checked==false)){
                    ...
                    rows[i].onclick=function(){
                        ...
                        if(rows.length==2){
                            ...
                            document.tmFormName.combinationalName.checked=true;

                        }
                        else{
                            document.tmFormName.combinationalName[l-1].checked=true;
                            changeColor(l-1);
                            } };}  }
       ...
       <script defer="pager">
        ...
        if("<s:property value="tmsNameAlreadyExist"/>" == "MPExisted"){
        alert("This target market has a media plan, so delete media plan first.");    
        }
        });
        </script>
    ...
    <div style="background-color: white;width: 950px;margin-top: 30px;margin-left: 10px;min-height: 400px;height: auto;border-radius: 30px 30px 0 0;"><br/>
       ...
       <div style="margin-top:38px;margin-left: 54px;position: absolute;font-family: Cambria;"><font style="font: bold 1.45em/1em Cambria;color:#595959;">My Account</font></div><br/>
        ...
        <table style="margin-left: 24px;margin-top:31px;position: absolute;">
            <tr><td style="border-bottom: 27px solid #7e7e7e;border-left: 15px solid transparent;border-right: 15px solid transparent;white-space: nowrap;"> <a href="tmStatesRedirection.action" title="Target Market Saving" style="position: relative;top:25px;cursor: pointer;text-decoration: none;" ><font color="white" style="font: bold 1.15em/1.25em Cambria;" onMouseOver="this.style.color='#00a9f1';" onMouseOut="this.style.color='white';"><b>&nbsp;Target Market Saving&nbsp;</b></font></a> </td>
            </tr>
        </table>
     ...
     <s:form  action="tmManagementSaving" name="tmFormName" id="tmFormId">


                <table>
                    <tr>
                        <td width="880px" align="right">
                            <div  style=" margin-top: 0px;margin-left: 0px;"> <font style="font-size: 14px;font-weight:bold;" color="#00a9f1">Total&nbsp;target markets&nbsp;:&nbsp;<font color="#292929"><span id="displayCount"></span></font></font></div>
                        </td>
                    </tr>
                </table>
                ...
                <display:table  id="insured_list"  class="tablesorter" name="tmsDetailsTakeFromDBToManagement">
                    <display:column headerClass="sortDisabled">
                        <input type="radio" style=" cursor: pointer;" onclick="changeColor(this);" name="combinationalName" value="${insured_list.tmsClientName},&, ${insured_list.tmsCampaignName},&, ${insured_list.tmsName},&, ${insured_list.geoName},&, ${insured_list.stateName},&, ${insured_list.targetedPeople},&, ${insured_list.tmId}"/>
                    </display:column>
                    <display:column property ="tmsClientName" title="Client Name"/>
                    <display:column property ="tmsCampaignName" title="Campaign Name" />
                    <display:column property ="tmsName" title="Target Market Name" />
                    <display:column style="white-space:normal;width:350px;float: left;text-align: justify;" property ="geoName" title="Selected Category" />
                    <display:column property ="stateName" title="Selected States" />
                    <display:column property ="targetedPeople" title="Targeted Value" />
        ...
        //alert("ON LOAD");
        var count="<s:property value="tmFormsCount"/>";
        // document.getElementById("cmdPrevId").style.display = "none";
       ...
       approve();
            ...
            function deleteForm(){
                ...
                for(var k=0; k < document.tmFormName.combinationalName.length; k++)
                {
                    if(document.tmFormName.combinationalName[k].checked){
                        combinationalNames=document.tmFormName.combinationalName[k].value;
                    }
                }
                if(document.tmFormName.combinationalName.checked){
                    combinationalNames=document.tmFormName.combinationalName.value;
                  }
                if(combinationalNames != ""){
                    combinationalNames = combinationalNames.split(", ");
                    var where_to= confirm("Do you want to delete Target Market "+combinationalNames[2]+" and Client "+combinationalNames[0]+" and Campaign "+combinationalNames[1]+"?");
                    if (where_to)
                    {
                        return true;
                    }
                    ...
                    window.alert("Please select a target market.");
                    return false;
                }
            }
            function edit(){
