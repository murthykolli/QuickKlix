...
<html>
    ...
    function checkAll()
    {
    if(document.tvCity.selectAll.checked== true)
    {
    for(var i=0; i < document.tvCity.tvSelectedCity.length; i++)
    {
    document.tvCity.tvSelectedCity[i].checked=true;
    }
    }
    else
    {
    for(var i=0; i < document.tvCity.tvSelectedCity.length; i++)
    {
    document.tvCity.tvSelectedCity[i].checked=false;
    } } }
    
    function checkStateAll(passStateName)
    {
    var stateChecked = document.getElementById(passStateName).checked;
    var len=document.tvCity.tvSelectedCity.length;
    if(stateChecked== true)
    {
    if(len==undefined){
    document.tvCity.tvSelectedCity.checked=true;
    }   
    for(var i=0; i < document.tvCity.tvSelectedCity.length; i++)
    {         
    if (document.tvCity.tvSelectedCity[i].className.indexOf(passStateName) != -1)
    document.tvCity.tvSelectedCity[i].checked = true;
    }
    }
    else
    {
    if(len==undefined && document.tvCity.tvSelectedCity.checked==true ){
    document.tvCity.tvSelectedCity.checked=false;
    }
    for (var i = 0; i < document.tvCity.tvSelectedCity.length; i++)
    {
    if (document.tvCity.tvSelectedCity[i].className.indexOf(passStateName) != -1)
    ...
    var tvSelectedCitys = "";
    for (var k = 0; k < document.tvCity.tvSelectedCity.length; k++)
    {
    if (document.tvCity.tvSelectedCity[k].checked) {
    tvSelectedCitys = document.tvCity.tvSelectedCity[k].value;
    }
    }
    if (document.tvCity.tvSelectedCity.checked) {
    tvSelectedCitys = document.tvCity.tvSelectedCity.value;
    }
    if (tvSelectedCitys == "") {
  ...
  <div style="margin-top:20px;margin-left: 30px;"><font style="font-family: Calibri;font-size: 22px;font-weight: bold;color:#6f6d6d;">Select by City - <font color="#00a9f1">Television</font></font></div><br/>
  <form name="tvCity" action="tvSelectByCity.action" method="post">
  ...
  <div class="mainBox" style="margin-left: 40px;margin-top:34px;background: white;min-height: 80px;width: 880px;border-radius: 12px 10px 0px 0px;height: auto;border: solid 2px #595959;">
       ...
       <tr>
        ...
        %>
        <td>
        <input type="checkbox" name="tvSelectedCity" class="<%=stateName%>" value="<%= cityNamesList.get(j)+"&&"+stateName  %>"/>
        </td>
        <td>
        <%= cityNamesList.get(j)  %>
        </td>
   ...
   <div align="center" style="margin-top: 15px;margin-left: 0px;">
         ...
         <table style="margin-left: 808px;margin-top: 0px;position: absolute">
             <tr><td><button type="submit" name="method:tvNamesByCity" onMouseOver="this.style.color='#00a9f1';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerWMOImg.png';" onMouseOut="this.style.color='white';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerImg.png';" style="background: #595959;border-radius: 7px;cursor: pointer; border: 1px solid #595959; color: white; height: 36px; font: bold 1.4em/1em Verdana,Geneva,Arial,Helvetica,sans-serif;box-shadow: 3px 3px 1px #999999;"  onclick="return citySelection();"><font style="top: -2px;position: relative;">NEXT</font> <img id="NextButtonImg1" src="/QuickKlix/images/SSImgs/TextPointerImg.png" style="position: relative;top: 1px;"/></button></td></tr>
         </table>
    </div>
