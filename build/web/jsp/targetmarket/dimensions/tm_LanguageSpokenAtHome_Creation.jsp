<%-- 
    Document   : tm_LanguageSpokenAtHome_Creation
    Created on : 7 Aug, 2014, 12:20:29 PM
    Author     : murthyk
--%>
...
<html>
    ...
    function checkAll(){
    if(document.languageFormName.selectAll.checked== true){
    for(var i=0; i < document.languageFormName.selectedLanguageCategories.length; i++){
    document.languageFormName.selectedLanguageCategories[i].checked=true;
    } }
    else{
    for(var i=0; i < document.languageFormName.selectedLanguageCategories.length; i++){
    document.languageFormName.selectedLanguageCategories[i].checked=false;
    } } }

    function checkStateAll(passStateName){
    var stateChecked = document.getElementById(passStateName).checked;
    var len=document.languageFormName.selectedLanguageCategories.length;

    if(stateChecked== true){
    if(len==undefined){
    document.languageFormName.selectedLanguageCategories.checked=true;
    }
    for(var i=0; i < document.languageFormName.selectedLanguageCategories.length; i++){
    if (document.languageFormName.selectedLanguageCategories[i].className.indexOf(passStateName) != -1){
    document.languageFormName.selectedLanguageCategories[i].checked = true;
    } }}
     
    else{
    if(len==undefined && document.languageFormName.selectedLanguageCategories.checked==true ){
    document.languageFormName.selectedLanguageCategories.checked=false;
    }
    ...
    var selectedLanguageCategoriess = "";
    for (var k = 0; k < document.languageFormName.selectedLanguageCategories.length; k++){
    if (document.languageFormName.selectedLanguageCategories[k].checked) {
    selectedLanguageCategoriess = document.languageFormName.selectedLanguageCategories[k].value;
    } }
    if (document.languageFormName.selectedLanguageCategories.checked) {
    selectedLanguageCategoriess = document.languageFormName.selectedLanguageCategories.value;
    }
    if (selectedLanguageCategoriess == "") {
    ...
    <body>
        ...
        <div style="margin-top:20px;margin-left: 30px;"><font style="font-family: Calibri;font-size: 22px;font-weight: bold;color:#6f6d6d;">Select by <font color="#00a9f1">Language Spoken At Home</font> Dimension - <font color="#00a9f1">Categories</font></font></div><br/>
        <form name="languageFormName" action="languageSaving" method="post">
        ...
        <%
        if(session.getAttribute("Language_Map") != null){
        Map sortedMap = (Map)session.getAttribute("Language_Map");
...
//        Collections.sort(dimensionNameAndCountList);
        ...
        <td><input type="checkbox" name="selectedLanguageCategories" class="<%=dimensionName%>" value="<%= pumaCount%>"/></td>
        ...
        <tr><td><button type="submit" name="method:languageDimensionSaving" onMouseOver="this.style.color='#00a9f1';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerWMOImg.png';" onMouseOut="this.style.color='white';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerImg.png';" style="background: #595959;border-radius: 7px;cursor: pointer; border: 1px solid #595959; color: white; height: 36px; font: bold 1.4em/1em Verdana,Geneva,Arial,Helvetica,sans-serif;box-shadow: 3px 3px 1px #999999;" onclick="return citySelection();"><font style="top: -2px;position: relative;">NEXT</font> <img id="NextButtonImg1" src="/QuickKlix/images/SSImgs/TextPointerImg.png" style="position: relative;top: 1px;"/></button></td></tr>
        </table>
        </div>
