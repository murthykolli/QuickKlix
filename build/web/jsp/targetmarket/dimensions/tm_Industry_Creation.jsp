<%-- 
    Document   : tm_Industry_Creation
    Created on : 7 Aug, 2014, 12:19:56 PM
    Author     : murthyk
--%>
...
<html>
    ...
    function checkAll(){
    if(document.industryFormName.selectAll.checked== true){
    for(var i=0; i < document.industryFormName.selectedIndustryCategories.length; i++){
    document.industryFormName.selectedIndustryCategories[i].checked=true;
    } }
    else{
    for(var i=0; i < document.industryFormName.selectedIndustryCategories.length; i++){
    document.industryFormName.selectedIndustryCategories[i].checked=false;
    } } }

    function checkStateAll(passStateName){
    var stateChecked = document.getElementById(passStateName).checked;
    var len=document.industryFormName.selectedIndustryCategories.length;

    if(stateChecked== true){
    if(len==undefined){
    document.industryFormName.selectedIndustryCategories.checked=true;
    }
    for(var i=0; i < document.industryFormName.selectedIndustryCategories.length; i++){
    if (document.industryFormName.selectedIndustryCategories[i].className.indexOf(passStateName) != -1){
    document.industryFormName.selectedIndustryCategories[i].checked = true;
    } }}
     
    else{
    if(len==undefined && document.industryFormName.selectedIndustryCategories.checked==true ){
    document.industryFormName.selectedIndustryCategories.checked=false;
    }
    ...
    var selectedIndustryCategoriess = "";
    for (var k = 0; k < document.industryFormName.selectedIndustryCategories.length; k++){
    if (document.industryFormName.selectedIndustryCategories[k].checked) {
    selectedIndustryCategoriess = document.industryFormName.selectedIndustryCategories[k].value;
    } }
    if (document.industryFormName.selectedIndustryCategories.checked) {
    selectedIndustryCategoriess = document.industryFormName.selectedIndustryCategories.value;
    }
    if (selectedIndustryCategoriess == "") {
    ...
    <body>
        ...
        <div style="margin-top:20px;margin-left: 30px;"><font style="font-family: Calibri;font-size: 22px;font-weight: bold;color:#6f6d6d;">Select by <font color="#00a9f1">Industry</font> Dimension - <font color="#00a9f1">Categories</font></font></div><br/>
        <form name="industryFormName" action="industrySaving" method="post">
        ...
        <%
        if(session.getAttribute("Industry_Map") != null){
        Map sortedMap = (Map)session.getAttribute("Industry_Map");
...
//        Collections.sort(dimensionNameAndCountList);
        ...
        <td><input type="checkbox" name="selectedIndustryCategories" class="<%=dimensionName%>" value="<%= pumaCount%>"/></td>
        ...
        <tr><td><button type="submit" name="method:industryDimensionSaving" onMouseOver="this.style.color='#00a9f1';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerWMOImg.png';" onMouseOut="this.style.color='white';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerImg.png';" style="background: #595959;border-radius: 7px;cursor: pointer; border: 1px solid #595959; color: white; height: 36px; font: bold 1.4em/1em Verdana,Geneva,Arial,Helvetica,sans-serif;box-shadow: 3px 3px 1px #999999;" onclick="return citySelection();"><font style="top: -2px;position: relative;">NEXT</font> <img id="NextButtonImg1" src="/QuickKlix/images/SSImgs/TextPointerImg.png" style="position: relative;top: 1px;"/></button></td></tr>
        </table>
        </div>
