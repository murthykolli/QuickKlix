<%-- 
    Document   : tm_AbilityToSpeakEnglish_Creation
    Created on : 7 Aug, 2014, 2:40:46 PM
    Author     : murthyk
--%>
...
<html>
    ...
    function checkAll(){
    if(document.abilityToSpeakFormName.selectAll.checked== true){
    for(var i=0; i < document.abilityToSpeakFormName.selectedAbilityCategories.length; i++){
    document.abilityToSpeakFormName.selectedAbilityCategories[i].checked=true;
    } }
    else{
    for(var i=0; i < document.abilityToSpeakFormName.selectedAbilityCategories.length; i++){
    document.abilityToSpeakFormName.selectedAbilityCategories[i].checked=false;
    } } }

    function checkStateAll(passStateName){
    var stateChecked = document.getElementById(passStateName).checked;
    var len=document.abilityToSpeakFormName.selectedAbilityCategories.length;

    if(stateChecked== true){
    if(len==undefined){
    document.abilityToSpeakFormName.selectedAbilityCategories.checked=true;
    }
    for(var i=0; i < document.abilityToSpeakFormName.selectedAbilityCategories.length; i++){
    if (document.abilityToSpeakFormName.selectedAbilityCategories[i].className.indexOf(passStateName) != -1){
    document.abilityToSpeakFormName.selectedAbilityCategories[i].checked = true;
    } } }
    
    else{
    if(len==undefined && document.abilityToSpeakFormName.selectedAbilityCategories.checked==true ){
    document.abilityToSpeakFormName.selectedAbilityCategories.checked=false;
    }
    for (var i = 0; i < document.abilityToSpeakFormName.selectedAbilityCategories.length; i++){
    if (document.abilityToSpeakFormName.selectedAbilityCategories[i].className.indexOf(passStateName) != -1){
    document.abilityToSpeakFormName.selectedAbilityCategories[i].checked = false;
    ...
    var selectedAbilityCategoriess = "";
    for (var k = 0; k < document.abilityToSpeakFormName.selectedAbilityCategories.length; k++){
    if (document.abilityToSpeakFormName.selectedAbilityCategories[k].checked) {
    selectedAbilityCategoriess = document.abilityToSpeakFormName.selectedAbilityCategories[k].value;
    } }
    if (document.abilityToSpeakFormName.selectedAbilityCategories.checked) {
    selectedAbilityCategoriess = document.abilityToSpeakFormName.selectedAbilityCategories.value;
    }
    if (selectedAbilityCategoriess == "") {
    ...
    <body>
        ...
        <div style="margin-top:20px;margin-left: 30px;"><font style="font-family: Calibri;font-size: 22px;font-weight: bold;color:#6f6d6d;">Select by <font color="#00a9f1">Ability To Speak English</font> Dimension - <font color="#00a9f1">Categories</font></font></div><br/>
        <form name="abilityToSpeakFormName" action="abilityToSpeakSaving" method="post">
        ...
        <%
        if(session.getAttribute("Ability_To_Speak_English_Map") != null){
        Map sortedMap = (Map)session.getAttribute("Ability_To_Speak_English_Map");
        ...
        <div class="mainBox" style="margin-left: 40px;margin-top:34px;background: white;min-height: 30px;width: 880px;border-radius: 12px 10px 0px 0px;height: auto;border: solid 2px #595959;">
        ...
        <div style="width:860px; height: 30px;margin-left: 7px;margin-top: 13px; overflow:auto;">
...
//        Collections.sort(dimensionNameAndCountList);
        ...
        <td><input type="checkbox" name="selectedAbilityCategories" class="<%=dimensionName%>" value="<%= pumaCount%>"/></td>
        ...
        <tr><td><button type="submit" name="method:abilityDimensionSaving" onMouseOver="this.style.color='#00a9f1';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerWMOImg.png';" onMouseOut="this.style.color='white';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerImg.png';" style="background: #595959;border-radius: 7px;cursor: pointer; border: 1px solid #595959; color: white; height: 36px; font: bold 1.4em/1em Verdana,Geneva,Arial,Helvetica,sans-serif;box-shadow: 3px 3px 1px #999999;" onclick="return citySelection();"><font style="top: -2px;position: relative;">NEXT</font> <img id="NextButtonImg1" src="/QuickKlix/images/SSImgs/TextPointerImg.png" style="position: relative;top: 1px;"/></button></td></tr>
        </table>
        </div>
