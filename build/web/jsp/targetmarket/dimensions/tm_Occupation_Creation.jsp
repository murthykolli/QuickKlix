<%-- 
    Document   : tm_Occupation_Creation
    Created on : 7 Aug, 2014, 12:22:29 PM
    Author     : murthyk
--%>
...
<html>
    ...
    function checkAll(){
    if(document.occupationFormName.selectAll.checked== true){
    for(var i=0; i < document.occupationFormName.selectedOccupationCategories.length; i++){
    document.occupationFormName.selectedOccupationCategories[i].checked=true;
    } }
    else{
    for(var i=0; i < document.occupationFormName.selectedOccupationCategories.length; i++){
    document.occupationFormName.selectedOccupationCategories[i].checked=false;
    } } }

    function checkStateAll(passStateName){
    var stateChecked = document.getElementById(passStateName).checked;
    var len=document.occupationFormName.selectedOccupationCategories.length;

    if(stateChecked== true){
    if(len==undefined){
    document.occupationFormName.selectedOccupationCategories.checked=true;
    }
    for(var i=0; i < document.occupationFormName.selectedOccupationCategories.length; i++){
    if (document.occupationFormName.selectedOccupationCategories[i].className.indexOf(passStateName) != -1){
    document.occupationFormName.selectedOccupationCategories[i].checked = true;
    } }}
     
    else{
    if(len==undefined && document.occupationFormName.selectedOccupationCategories.checked==true ){
    document.occupationFormName.selectedOccupationCategories.checked=false;
    }
    ...
    var selectedOccupationCategoriess = "";
    for (var k = 0; k < document.occupationFormName.selectedOccupationCategories.length; k++){
    if (document.occupationFormName.selectedOccupationCategories[k].checked) {
    selectedOccupationCategoriess = document.occupationFormName.selectedOccupationCategories[k].value;
    } }
    if (document.occupationFormName.selectedOccupationCategories.checked) {
    selectedOccupationCategoriess = document.occupationFormName.selectedOccupationCategories.value;
    }
    if (selectedOccupationCategoriess == "") {
    ...
    <body>
        ...
        <div style="margin-top:20px;margin-left: 30px;"><font style="font-family: Calibri;font-size: 22px;font-weight: bold;color:#6f6d6d;">Select by <font color="#00a9f1">Occupation</font> Dimension - <font color="#00a9f1">Categories</font></font></div><br/>
        <form name="occupationFormName" action="occupationSaving" method="post">
        ...
        <%
        if(session.getAttribute("Occupation_Map") != null){
        Map sortedMap = (Map)session.getAttribute("Occupation_Map");
...
//        Collections.sort(dimensionNameAndCountList);
        ...
        <td><input type="checkbox" name="selectedOccupationCategories" class="<%=dimensionName%>" value="<%= pumaCount%>"/></td>
