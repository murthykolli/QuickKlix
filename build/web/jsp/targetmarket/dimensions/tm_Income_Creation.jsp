<%-- 
    Document   : tm_Income_Creation
    Created on : 7 Aug, 2014, 12:19:27 PM
    Author     : murthyk
--%>
...
<html>
    ...
    function checkAll(){
    if(document.incomeFormName.selectAll.checked== true){
    for(var i=0; i < document.incomeFormName.selectedIncomeCategories.length; i++){
    document.incomeFormName.selectedIncomeCategories[i].checked=true;
    } }
    else{
    for(var i=0; i < document.incomeFormName.selectedIncomeCategories.length; i++){
    document.incomeFormName.selectedIncomeCategories[i].checked=false;
    } } }

    function checkStateAll(passStateName){
    var stateChecked = document.getElementById(passStateName).checked;
    var len=document.incomeFormName.selectedIncomeCategories.length;

    if(stateChecked== true){
    if(len==undefined){
    document.incomeFormName.selectedIncomeCategories.checked=true;
    }
    for(var i=0; i < document.incomeFormName.selectedIncomeCategories.length; i++){
    if (document.incomeFormName.selectedIncomeCategories[i].className.indexOf(passStateName) != -1){
    document.incomeFormName.selectedIncomeCategories[i].checked = true;
    } }}
     
    else{
    if(len==undefined && document.incomeFormName.selectedIncomeCategories.checked==true ){
    document.incomeFormName.selectedIncomeCategories.checked=false;
    }
    ...
    var selectedIncomeCategoriess = "";
    for (var k = 0; k < document.incomeFormName.selectedIncomeCategories.length; k++){
    if (document.incomeFormName.selectedIncomeCategories[k].checked) {
    selectedIncomeCategoriess = document.incomeFormName.selectedIncomeCategories[k].value;
    } }
    if (document.incomeFormName.selectedIncomeCategories.checked) {
    selectedIncomeCategoriess = document.incomeFormName.selectedIncomeCategories.value;
    }
    if (selectedIncomeCategoriess == "") {
    ...
    <body>
        ...
        <div style="margin-top:20px;margin-left: 30px;"><font style="font-family: Calibri;font-size: 22px;font-weight: bold;color:#6f6d6d;">Select by <font color="#00a9f1">Income</font> Dimension - <font color="#00a9f1">Categories</font></font></div><br/>
        <form name="incomeFormName" action="incomeSaving" method="post">
        ...
        <%
        if(session.getAttribute("Income_Map") != null){
        Map sortedMap = (Map)session.getAttribute("Income_Map");
        ...
        <div class="mainBox" style="margin-left: 40px;margin-top:34px;background: white;min-height: 180px;width: 880px;border-radius: 12px 10px 0px 0px;height: auto;border: solid 2px #595959;">
        ...
        <div style="width:860px; height: 180px;margin-left: 7px;margin-top: 13px; overflow:auto;">
...
//        Collections.sort(dimensionNameAndCountList);
        ...
        <td><input type="checkbox" name="selectedIncomeCategories" class="<%=dimensionName%>" value="<%= pumaCount%>"/></td>
        ...
        <tr><td><button type="submit" name="method:incomeDimensionSaving" onMouseOver="this.style.color='#00a9f1';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerWMOImg.png';" onMouseOut="this.style.color='white';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerImg.png';" style="background: #595959;border-radius: 7px;cursor: pointer; border: 1px solid #595959; color: white; height: 36px; font: bold 1.4em/1em Verdana,Geneva,Arial,Helvetica,sans-serif;box-shadow: 3px 3px 1px #999999;" onclick="return citySelection();"><font style="top: -2px;position: relative;">NEXT</font> <img id="NextButtonImg1" src="/QuickKlix/images/SSImgs/TextPointerImg.png" style="position: relative;top: 1px;"/></button></td></tr>
        </table>
        </div>
