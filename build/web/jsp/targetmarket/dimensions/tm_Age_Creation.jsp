<%-- 
    Document   : tm_Age_Creation
    Created on : 7 Aug, 2014, 12:16:02 PM
    Author     : murthyk
--%>
...
<html>
    ...
    function checkAll(){
    if(document.ageFormName.selectAll.checked== true){
    for(var i=0; i < document.ageFormName.selectedAgeCategories.length; i++){
    document.ageFormName.selectedAgeCategories[i].checked=true;
    } }
    else{
    for(var i=0; i < document.ageFormName.selectedAgeCategories.length; i++){
    document.ageFormName.selectedAgeCategories[i].checked=false;
    } } }

    function checkStateAll(passStateName){
    var stateChecked = document.getElementById(passStateName).checked;
    var len=document.ageFormName.selectedAgeCategories.length;

    if(stateChecked== true){
    if(len==undefined){
    document.ageFormName.selectedAgeCategories.checked=true;
    }
    for(var i=0; i < document.ageFormName.selectedAgeCategories.length; i++){
    if (document.ageFormName.selectedAgeCategories[i].className.indexOf(passStateName) != -1){
    document.ageFormName.selectedAgeCategories[i].checked = true;
    } }}
     
    else{
    if(len==undefined && document.ageFormName.selectedAgeCategories.checked==true ){
    document.ageFormName.selectedAgeCategories.checked=false;
    }
    ...
    var selectedAgeCategoriess = "";
    for (var k = 0; k < document.ageFormName.selectedAgeCategories.length; k++){
    if (document.ageFormName.selectedAgeCategories[k].checked) {
    selectedAgeCategoriess = document.ageFormName.selectedAgeCategories[k].value;
    } }
    if (document.ageFormName.selectedAgeCategories.checked) {
    selectedAgeCategoriess = document.ageFormName.selectedAgeCategories.value;
    }
    if (selectedAgeCategoriess == "") {
    ...
    <body>
        ...
        <div style="margin-top:20px;margin-left: 30px;"><font style="font-family: Calibri;font-size: 22px;font-weight: bold;color:#6f6d6d;">Select by <font color="#00a9f1">Age</font> Dimension - <font color="#00a9f1">Categories</font></font></div><br/>
        <form name="ageFormName" action="ageSaving" method="post">
        ...
        <%
        if(session.getAttribute("Age_Map") != null){
        Map sortedMap = (Map)session.getAttribute("Age_Map");
...
//        Collections.sort(dimensionNameAndCountList);
        ...
        <td><input type="checkbox" name="selectedAgeCategories" class="<%=dimensionName%>" value="<%= pumaCount%>"/></td>
        ...
        <tr><td><button type="submit" name="method:ageDimensionSaving" onMouseOver="this.style.color='#00a9f1';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerWMOImg.png';" onMouseOut="this.style.color='white';document.getElementById('NextButtonImg1').src='/QuickKlix/images/SSImgs/TextPointerImg.png';" style="background: #595959;border-radius: 7px;cursor: pointer; border: 1px solid #595959; color: white; height: 36px; font: bold 1.4em/1em Verdana,Geneva,Arial,Helvetica,sans-serif;box-shadow: 3px 3px 1px #999999;" onclick="return citySelection();"><font style="top: -2px;position: relative;">NEXT</font> <img id="NextButtonImg1" src="/QuickKlix/images/SSImgs/TextPointerImg.png" style="position: relative;top: 1px;"/></button></td></tr>
        </table>
        </div>
