<%-- 
    Document   : tm_Gender_Creation
    Created on : 7 Aug, 2014, 12:18:47 PM
    Author     : murthyk
--%>
...
<html>
    ...
    function checkAll(){
    if(document.genderFormName.selectAll.checked== true){
    for(var i=0; i < document.genderFormName.selectedGenderCategories.length; i++){
    document.genderFormName.selectedGenderCategories[i].checked=true;
    } }
    else{
    for(var i=0; i < document.genderFormName.selectedGenderCategories.length; i++){
    document.genderFormName.selectedGenderCategories[i].checked=false;
    } } }

    function checkStateAll(passStateName){
    var stateChecked = document.getElementById(passStateName).checked;
    var len=document.genderFormName.selectedGenderCategories.length;

    if(stateChecked== true){
    if(len==undefined){
    document.genderFormName.selectedGenderCategories.checked=true;
    }
    for(var i=0; i < document.genderFormName.selectedGenderCategories.length; i++){
    if (document.genderFormName.selectedGenderCategories[i].className.indexOf(passStateName) != -1){
    document.genderFormName.selectedGenderCategories[i].checked = true;
    } }}
     
    else{
    if(len==undefined && document.genderFormName.selectedGenderCategories.checked==true ){
    document.genderFormName.selectedGenderCategories.checked=false;
    }
    ...
    var selectedGenderCategoriess = "";
    for (var k = 0; k < document.genderFormName.selectedGenderCategories.length; k++){
    if (document.genderFormName.selectedGenderCategories[k].checked) {
    selectedGenderCategoriess = document.genderFormName.selectedGenderCategories[k].value;
    } }
    if (document.genderFormName.selectedGenderCategories.checked) {
    selectedGenderCategoriess = document.genderFormName.selectedGenderCategories.value;
    }
    if (selectedGenderCategoriess == "") {
    ...
    <body>
        ...
        <div style="margin-top:20px;margin-left: 30px;"><font style="font-family: Calibri;font-size: 22px;font-weight: bold;color:#6f6d6d;">Select by <font color="#00a9f1">Gender</font> Dimension - <font color="#00a9f1">Categories</font></font></div><br/>
        <form name="genderFormName" action="genderSaving" method="post">
        ...
        <%
        if(session.getAttribute("Gender_Map") != null){
        Map sortedMap = (Map)session.getAttribute("Gender_Map");
...
//        Collections.sort(dimensionNameAndCountList);
        ...
        <td><input type="checkbox" name="selectedGenderCategories" class="<%=dimensionName%>" value="<%= pumaCount%>"/></td>
